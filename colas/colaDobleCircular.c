...
int eliminaElementoColaA(COLA *cola);
int eliminaElementoColaD(COLA *cola);
    ...
    do
    {
        ...
        switch(opcion)
        {
            ...
            case 2:
                ...
                if(cola == NULL)
                    printf("\nNOse puede eliminar. Tu cola esta vacia...");

                printf("\nPor donde quieres eliminar, Adelante o Detras (A/D): ");
                getchar();
                scanf("%c", &respuesta);

                if(respuesta == 'A' || respuesta == 'a')
                {
                    validaEliminar = eliminaElementoColaA(cola);
                    if(validaEliminar == 1)
                ...
                {
                    validaEliminar = eliminaElementoColaD(cola);
                    if(validaEliminar == 1)
                        printf("\nSe elimino correctamente...");
                    else
                        printf("\nHubo un error al eliminar");
...
int eliminaElementoColaA(COLA *cola)
{
    ...
    if(colaVacia(cola) == 1)
    {
        printf("\nNose puede eliminar. Cola vacia");
        return 0;
    }

    elementoAux = cola->frente;
    cola->frente->siguiente->anterior = cola->final;
    cola->final->siguiente = cola->frente->siguiente;
...
int eliminaElementoColaD(COLA *cola)
{
    ...
    elementoAux = cola->final;
    cola->frente->anterior = cola->final->anterior;
    cola->final->anterior->siguiente = cola->frente;
    cola->final = elementoAux->anterior;
    free(elementoAux);
    return 1;
...
void imprimirCola(ELEMENTOCOLA *cola, ELEMENTOCOLA *elemenAux)
{
    ...
    if(cola == NULL)
    {
        printf("\nNo se puede imprimir. La cola esta vacia...");
        return;
    }
